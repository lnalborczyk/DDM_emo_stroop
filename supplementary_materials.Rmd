---
title: "Supplementary materials to 'Different processing of spatial frequencies in autism spectrum compared to control suggested by an emotional stroop with filtered primes'"
...
affiliation:
  ...
  - id: 2
    institution: GIPSA-lab, CNRS, Univ. Grenoble Alpes, 38000, Grenoble, France.
  - id: 3
    institution: UMR 1253 iBrain, Université de Tours, Inserm, Tours, France
  - id: 4
    institution: Fondation Ophtalmologique A. de Rothschild, Paris, France.
  - id: 5
...
```{r correctness1, echo = FALSE, fig.cap = "Distribution of RTs for correct and incorrect responses according to Sex and Group. Incorrect responses are in pink and correct responses in blue."}
df %>%
...
# recoding the binary predictors using contrasts (-0.5 vs. 0.5)
df <- df %>%
  mutate(
...
varying_effects <- brm(
  ...
  file = "models/wiener_varying_effects.rds",
...
varying_effects_prime <- readRDS("models/wiener_varying_effects_prime_bias.rds")

# performing model comparison via approximate LOO-CV
# varying_varying <- add_criterion(varying_effects, "waic")
# varying_effects_prime <- add_criterion(varying_effects_prime, "waic")

# displays the model comparison table
# LOOCV <- loo_compare(hvarying_effects, varying_effects_prime, criterion = "waic")
...
# prints the table
apa_table(
  model_comparison_table %>% data.frame %>% rownames_to_column(var = "model"),
  placement = "htb",
  align = rep("c", 9),
...
hyp <- hypothesis(varying_effects, "GroupC:CongC = 0")
...
- participants with accuracy < 50 % (1 autistic participant)  
- trials with RT < 300 ms (1,50 %)  
...
apa_table(
  ...
  caption = "Expected log pointise predictive density (ELPD) and LOOIC for each BMLM on RT.",
  # note = "For each model (for each line), the first two columns represent the estimated...",
  escape = TRUE,
  font_size = "tiny"
  )
...
As reported on *Table \@ref(tab:modelComp1RT)*, `BMgcpe` appeared to be the best model compared with models without emotion or with sex (full model or parcimonious model). Hence, we kept this model and then added covariates to this model.  
...
apa_table(
  ...
  align = rep("c", 9),
  digits = c(0, 3, 3, 3, 3, 3, 3, 3, 3, 3),
  caption = "Expected log pointise predictive density (ELPD) and LOOIC for each of the four BMLMs on RT, with covariates",
  # note = "For each model (for each line), the first two columns represent the estimated...",
  escape = TRUE,
  font_size = "tiny"
)
...
placement = "htb",
align = c("c", "c"),
caption = "Estimates and BF01 from the BMLM on RT.",
...
We checked the quality of this model with posterior predictive checking, which seemed to be good (*Figure 4* of the main paper associated to this supplementary material). The results of this BMLM are summarized in *Table \@ref(tab:summaryBMgcpe)* and are described in the paper.

\newpage

# Removing outliers

We also ran our model removing participants who were outliers (results below Quartile1 – 1.5 * InterQuartile range or above Quartile3 +1.5 * InterQuartile Rang) on at least half of the condition for accuracy or for RT (3 ASD - 2 females, 1 male - and 3 TD - 2 females, 1 male, as preregistered on <https://osf.io/345jv>.
...
```{r demoWO, result = "asis", out.width = "\\textwidth"}
apa_table(
    demoWO_stat,
    placement = "htb",
    align = c("c", "c"),
...
```{r ppcheckBMwo, out.width = "\\textwidth", fig.pos = "htb", fig.width = 4, fig.height = 2.5, fig.cap = "Posterior predictive checking for the BMLM without outliers."}
...
placement = "htb",
align = c("c", "c"),
caption = "Estimates from the ExGaussian BMLM on RT without outliers.",
small = TRUE,
escape = TRUE
)
...
#save(MWIES1, file = "Model/MWIES1.rda")
...
apa_table(
  ...
  align = rep("c", 9),
  digits = c(0, 3, 3, 3, 3, 3, 3, 3, 3, 3),
  caption = "Expected log pointise predictive density (ELPD) and LOOIC for each of the three BMLM on IES.",
  # note = "For each model (for each line), the first two columns represent the estimated...",
  escape = TRUE,
  font_size = "tiny"
  )
...
The model `IBMgcpe` seemed to be the best one (see *Table \@ref(tab:modelCompIES1)*) even if the parsimonious model included sex is quite good as well.
...
apa_table(
  ...
  align = rep("c", 9),
  digits = c(0, 3, 3, 3, 3, 3, 3, 3, 3, 3),
  caption = "Expected log pointise predictive density (ELPD) and LOOIC for each of the four BMLM on IES, with covariates.",
  # note = "For each model (for each line), the first two columns represent the estimated...",
  escape = TRUE,
  font_size = "tiny"
  )
...
```
 ...
 The model `IBMf`, with FSIQ as a covariate, appear to be the best model (see *Table \@ref(tab:modelCompIES2)*). We checked the validity of this model with posterior predictive check (*Figure 2*), which appeared to be good. 
 
## Posterior predictive check
 
```{r ppcheckIBMf, out.width = "\\textwidth", fig.pos = "!h", fig.width = 4, fig.height = 3, fig.cap = "Posterior predictive checking for BMLM concerning the IES."}
...
#placement = "!H",
align = c("c", "c"),
caption = "Estimates from the ExGaussian BMLM concerning the IES.",
small = TRUE,
escape = TRUE
)
